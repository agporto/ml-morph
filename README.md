...
## Usage
**_ml-morph_** uses a combination of object detection with shape prediction to perform automated landmarking in images of semi-rigid biological structures. To generate such detectors and predictors, we need to train machine learning models using manually annotated datasets. These manually annotated datasets can be generated using the `imglab` tool, or (alternatively) can also be generated by converting traditional morphometric landmark files (`tpsDig` and `standard XY coordinate`).
 
The pipeline itself has four mains components, which should be applied in sequence: 
1) Preprocessing (_preprocessing.py_)
2) Training and Testing object detectors (_detector_trainer.py / detector_tester.py_)
3) Training and testing shape predictors (_shape_trainer.py / shape_tester.py_)
4) Predicting the landmark positions in a new set of images (_prediction.py_) 
